...
public class AudioParallelRenderer {
     ...
     *
     * @param   callback
     *          Optional. Function to be called when this asynchronous method finishes. It must have the signature:
     *          function (success : Boolean, reason : String) : void;
     *
     *          If decommissioning is successful, then the callback is called with `true` and `null` as arguments; if
     *          the process fails, the callback is called with `false` and a String describing the reason for the
     *          failure.
     *
